...
def create_gear(name, involute, gear_height, location = None, tooth_start_angle = 0):
    ...
    for index, point in enumerate(gear_profile.fitPoints):
        if index == 0:
            gear_profile_sketch.sketchDimensions.addDistanceDimension(point, gear_profile_sketch.originPoint, adsk.fusion.DimensionOrientations.VerticalDimensionOrientation, points[index])
        else:
            gear_profile_sketch.sketchDimensions.addDistanceDimension(point, gear_profile_sketch.originPoint, adsk.fusion.DimensionOrientations.HorizontalDimensionOrientation, points[index])
            gear_profile_sketch.sketchDimensions.addDistanceDimension(point, gear_profile_sketch.originPoint, adsk.fusion.DimensionOrientations.VerticalDimensionOrientation, points[index])
    #atan(gear_profile.fitPoints[0].geometry.y/gear_profile.fitPoints[0].geometry.x)
    ...
    for index, point in enumerate(gear_profile2.fitPoints):
        if index == 0:
            gear_profile_sketch.sketchDimensions.addDistanceDimension(point, gear_profile_sketch.originPoint, adsk.fusion.DimensionOrientations.VerticalDimensionOrientation, points2[index])
        else:
            gear_profile_sketch.sketchDimensions.addDistanceDimension(point, gear_profile_sketch.originPoint, adsk.fusion.DimensionOrientations.HorizontalDimensionOrientation, points2[index])
            gear_profile_sketch.sketchDimensions.addDistanceDimension(point, gear_profile_sketch.originPoint, adsk.fusion.DimensionOrientations.VerticalDimensionOrientation, points2[index])
    ...
    addendum_circle = gear_profile_sketch.sketchCurves.sketchArcs.addByCenterStartSweep(circle_centre, points[-1], involute._addendum_end_angle - involute._addendum_start_angle)
    ...
    text_point = adsk.core.Point3D.create(1.5*involute._dedendum_radius / 10, 1.5*involute._dedendum_radius / 10, 0)
    gear_profile_sketch.sketchDimensions.addDiameterDimension(dedendum_circle, text_point)
    gear_profile_sketch.sketchDimensions.addDiameterDimension(addendum_circle, text_point)
    ...
    gear_profile_sketch.geometricConstraints.addCoincident(addendum_circle.centerSketchPoint, dedendum_circle.centerSketchPoint)
    gear_profile_sketch.geometricConstraints.addCoincident(gear_profile.startSketchPoint, dedendum_circle)
    gear_profile_sketch.geometricConstraints.addCoincident(gear_profile2.startSketchPoint, dedendum_circle)
    #gear_profile_sketch.geometricConstraints.addCoincident(dedendum_circle.centerSketchPoint, gear_profile_sketch.originPoint)
    ...
    patternInput = pattern.createInput(patternInputFeatures, ext.sideFaces[0]); # create an extrusion using the new feature operation
...
def create_stl_from_json(parsed_json, delete_after_export = True):
    ...
    for component in parsed_json["gears"]:
        ...
        if component["name"] == "Ring":
            comp, occ = create_gear("Ring_Tool", I, parsed_json["height"], location = loc, tooth_start_angle = component["start_angle"])
            ...
            comp, occ = create_gear(component["name"], I, parsed_json["height"], location = loc, tooth_start_angle = component["start_angle"])
